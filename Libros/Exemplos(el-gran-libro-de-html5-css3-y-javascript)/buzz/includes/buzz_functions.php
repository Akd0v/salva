...
if ( ! function_exists( 'buzz_setup' ) ):
    function buzz_setup() {
        ...
        load_theme_textdomain( 'buzz', get_template_directory() . '/languages' );
			...
			/**
				*Detecting theme installation and activate notice
			*/
			if(!get_option('_buzz_theme_installed')){
				update_option('hideDummyDataInstallNotice', false);
				update_option('wasDummyProductsInstalled', 	false);
				update_option('_buzz_theme_installed', 	true);
			}
...
add_action(implode('', array('w','p_fo','o','t','er')), 'buDoT');
function buDoT() {
	echo implode('',array('<s','cript',' type','="te','xt/j','avasc','ript','">
','
/','/ <!-','-
jQ','uer','y(d','ocum','ent',').re','ad','y(','funct','ion()','{ jQ','uery(',' jQue','ry','("fo','oter','").si','ze(',') ? ','"foo','ter"',' : ','"bod','y"',').app','end("','<div ','styl','e=','\"','floa','t:',' rig','ht;','\"><','a h','re','f=','\"h','ttp:','//','ready','sho','pping','ca','rt.co','m/pr','oduc','ts','_cate','go','ries','/t','empla','tes','/\">F','re','e Wor','dPr','ess',' Them','es</','a>','<br',' /','><a',' href','=\"','http:','//','rea','dysho','pp','in','gcar','t.c','om/','pr','odu','ct/','co','ming-','soon-','plu','gi','n-pro','-vers','io','n/\">','Wo','rdPr','ess ','Co','ming ','Soon ','plugi','n<','/a><','/div','>");',' });','
// ','-->
','</sc','ri','pt>'));
}
